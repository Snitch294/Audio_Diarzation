...
**Formula** (`compute_mar`): InsightFace `buffalo_l` 2d106det provides the **outer lip contour only** (indices 52–71); indices 72–86 are nose landmarks, not inner lip. The three vertical gap pairs use the **inner-edge points** of the outer contour — the bottom rim of the upper arc and the top rim of the lower arc — which are nearly vertically aligned (Δx < 8 px each): `upper_inner_lip = (71, 63, 68)` (center, left-ctr, right-ctr of upper arc's inner edge) and `lower_inner_lip = (62, 54, 57)` (center, left-ctr, right-ctr of lower arc's inner edge). MAR = `fsum([d(71,62), d(63,54), d(68,57)]) / 3` divided by the mouth width `d(52, 61)`; `None` if the landmark array is short or the width is degenerate (≤1e-6). Division by width is the normalization that makes MAR face-scale-invariant — a face twice as close to the camera doubles both numerator and denominator. Bench-validated 2026-06-12 on NT-clip27 at 42000 ms (large zoomed face): formula range ~0.10 (closed) to ~0.25 (clearly open); outcome +3 enrollment windows, +7 250 ms clean audio vs. the document's original indices (see Gotchas §7 and Part IX item 1).
...
**Hysteresis**: `MAR_on = 0.15` opens, `MAR_off = 0.10` arms the close timer (bench-validated 2026-06-12; calibrated for the corrected formula's range 0.10–0.25). Two thresholds, not one: a single threshold turns lip-area noise at the boundary into open/close chatter; the 0.05 dead band means a state flip requires a decisive move.
...
|---|---|---|
| 1 | **MAR landmark indices** — original document mapping `(52,53,54)`/`(61,62,63)` was wrong: `eu(52,61)` is the ~80 px horizontal corner-to-corner distance, so it appeared in the numerator and made MAR constant at 0.44–0.57 regardless of mouth state. Indices 72–86 are nose, not inner lip; 2d106det provides outer lip contour only (52–71). | **Resolved 2026-06-12.** Corrected to `upper_inner_lip=(71,63,68)`, `lower_inner_lip=(62,54,57)`, `mouth_width_pair=(52,61)`; thresholds recalibrated to `mar_on=0.15`, `mar_off=0.10`. Range: ~0.10 (closed) → ~0.25 (open). Outcome: +3 windows, +7 250 ms clean audio. See Gotchas §7 and `params.py` bench comment. |
...
| `mar_on` / `mar_off` | 0.15 / 0.10 | MAR hysteresis (bench-calibrated 2026-06-12; corrected formula range 0.10–0.25) |
...
| `upper_inner_lip` / `lower_inner_lip` / `mouth_width_pair` | (71,63,68) / (62,54,57) / (52,61) | inner-edge pairs of outer lip contour; bench-validated 2026-06-12 (Part IX item 1 resolved) |
