...
| `onnxruntime_cuda_provider_missing` | CUDAExecutionProvider not even registered (CPU-only ORT wheel) | re-download ORT via the CUDA-12 feed (FLAG 2); apply §5 exports |
| `onnxruntime_cuda_provider_inactive` | ORT's CUDA provider is *registered* but its `.so` fails to load, so a real session silently falls back to CPU (CUDA-11.8 ORT wheel from PyPI, or §5 `LD_LIBRARY_PATH` missing the cu12 `cufft`/`cudart` libs) | reinstall ORT from the CUDA-12 feed with `--index-url` (Gotchas #2); `pip install nvidia-cufft-cu12==11.0.2.54 nvidia-cuda-runtime-cu12==12.1.105`; apply §5 exports |
...
- *2026-06-15 — Bring-up correction (observed on a fresh `.venv` build): the claim that `nvidia-cufft-cu12`/`nvidia-cuda-runtime-cu12` are **auto-vendored** by `torch==2.1.2+cu121` did NOT hold — the torch wheel bundles only cublas/cudnn into `torch/lib`; the `nvidia/` site-packages dir was empty, so §5's `cufft/lib` and `cuda_runtime/lib` paths resolved to nothing and ORT halted with `libcufft.so.11 ... not found`. **Run §5's explicit `pip install nvidia-cufft-cu12==11.0.2.54 nvidia-cuda-runtime-cu12==12.1.105 nvidia-cublas-cu12==12.1.3.1` as a required step, not belt-and-suspenders.** Also confirmed: `requirements.txt`'s `--extra-index-url https://aka.ms/onnxruntime-cuda-12` loses the version tie to PyPI and installs the **CUDA-11.8** ORT build (`libcublasLt.so.11`), which silently runs InsightFace on CPU — reinstall ORT with `--index-url` per Gotchas #2.*
- *2026-06-15 — Gate hardening (FLAG 2): `check_onnxruntime_cuda` no longer trusts `get_available_providers()` alone (CUDAExecutionProvider stays *registered* even when its `.so` cannot load). It now instantiates a real `InferenceSession` on the vendored `buffalo_l/det_10g.onnx` and asserts the active session provider is CUDA — catching the silent-CPU-fallback state that previously passed the gate. New halt reasons: `onnxruntime_cuda_provider_inactive` (registered but session fell back to CPU / failed to build) and `onnxruntime_cuda_probe_missing`. Stdlib self-test unaffected (the probe needs ORT + the store, exercised only under `--run`).*
